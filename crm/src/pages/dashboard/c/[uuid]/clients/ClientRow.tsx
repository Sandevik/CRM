...
export default function ClientRow({client}: {client: Client}) {
  ...
  return (
    <li className="grid grid-cols-2 md:grid-cols-5 bg-background-light text-lg bg-opacity-60 hover:bg-background-light transition-colors">
        ...
        <Link href={`/dashboard/c/${crm?.crmUuid}/clients/${client.uuid}`} className="hidden md:block border-l-2 p-2 pl-4 border-background-light truncate">{client.phoneNumber || "-"}</Link>
        <Link href={`/dashboard/c/${crm?.crmUuid}/clients/${client.uuid}`} className="hidden md:block border-l-2 p-2 pl-4 border-background-light truncate">{client.city}</Link>
        <div className="border-l-2 p-2 pl-4 border-background-light truncate gap-2 items-center hidden md:flex">{ hasCopied ? <BsClipboard2CheckFill className="text-3xl cursor-pointer" onClick={() => copyToClipBoard(client.uuid)} /> : <BsClipboard2Fill className="text-3xl cursor-pointer" onClick={() => copyToClipBoard(client.uuid)} />} <div className="truncate">{client.uuid}</div></div>
    </li>
  )
}
