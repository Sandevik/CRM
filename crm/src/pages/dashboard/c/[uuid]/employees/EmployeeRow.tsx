...
export default function EmployeeRow({employee}: {employee: Employee}) {
  ...
  return (
    <li className="grid grid-cols-2 md:grid-cols-5 bg-background-light text-lg bg-opacity-60 hover:bg-background-light transition-colors">
        ...
        <Link href={`/dashboard/c/${crm?.crmUuid}/employees/${employee.uuid}`} className="hidden md:block border-l-2 p-2 pl-4 border-background-light truncate">{employee.contract_uuid ? <Text text={{eng: "Yes", swe: "Ja"}}/> : <Text text={{eng: "No", swe: "Nej"}} />}</Link>
    </li>
  )
}
