...
export default function Task({task}: {task: Task}) {
  const percentage:number = (100-(task?.percentage || 100)) > 100 ? 100 : (100-(task?.percentage || 100)) < 0 ? 0 : (100-(task?.percentage || 100));
...
const LoadingBar = ({percentage}: {percentage: number}) => {
  return (
    <div className={`absolute bottom-2 h-1 bg-white w-full -m-2 `}>
      <div style={{"--percentage": percentage.toString()+"%"} as any} className={`h-1 w-full ${percentage === 100 ? "rounded-r-md bg-light-red left-0" : "rounded-none bg-blue-500 left-[-100%] translate-x-[var(--percentage)]"} absolute bottom-0  `}></div>
    </div>
  )
}