...
export default function TruckGraphic({truck, loadedWeight, overLoaded}: {truck: Truck, loadedWeight: number, overLoaded: boolean}) {
  ...
  return (
    <div className='relative flex justify-between' style={{width: truck.length / 19, aspectRatio: truck.length / truck.width}}>
            ...
            <ul style={{width : (truck.cargoLength || truck.length - truck.length * 0.2) / 19, aspectRatio: ((truck.cargoLength || truck.length - truck.length * 0.2) / truck.width)}} className={`p-0.5 flex flex-wrap items-center gap-0.5 place-items-center bg-background-light -skew-x-[40deg] before:content-[''] before:w-9 before:absolute before:right-0 before:translate-x-6 before:skew-x-[-45deg] before:top-6 before:h-1 before:bg-light-red  before:rotate-[-90deg] after:content-[''] after:w-9 after:absolute after:right-0 after:translate-x-6 after:skew-x-[-45deg] after:top-[7.4rem] after:h-1 after:bg-light-red  after:rotate-[-90deg]`}>
            {truck.width && truck.cargoLength && 
              Array(calculatePalletNumber(truck)).fill("").map((_,i) => (<li style={{height: 800 / 19, aspectRatio: 1200 / 800}} className={`${overLoaded ? "bg-light-red" : "bg-accent-color"} transition-colors z-10 flex items-center justify-center text-background-dark ${i % 3 && "pl-1"}`}>{(loadedWeight / calculatePalletNumber(truck as Truck)).toFixed(1) !== "0.0" && <div className="text-sm -rotate-[25deg] skew-x-[34deg]">{(loadedWeight / calculatePalletNumber(truck as Truck)).toFixed(1)}kg</div> } </li>))
            }
            </ul>
            {Array(truck.axleCount).fill("").map((_, i) => (
              <div style={{translate: ((getWheelLocation(i) / 19)).toFixed(0)+"px"}} className={`absolute h-12 rounded-full aspect-square bg-background-light z-10 bottom-0 translate-y-[52px] skew-x-[6deg]`}></div>
              ))}
          </div>
  )
}
