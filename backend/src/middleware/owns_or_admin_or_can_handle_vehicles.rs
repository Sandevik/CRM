                    ...
                    match user {
                        ...
                        Some(user) => {
                            if user.current_jwt == Some(token_string) {
                                if CRM::user_owns_or_is_admin(&user, &uuid, data).await.unwrap() || CRM::user_can_handle_vehicles(&user, &uuid, data).await.expect("ERROR: Could not determine if user can handle vehicles") {
                                    let (_, mut payload) = actix_http::h1::Payload::create(true);
